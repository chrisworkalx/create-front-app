...
export default function (value, actions) {
  ...
  switch (actionType) {
    case 'get':
      ...
      if (!v)
        return console.log(
          `${logSymbols.warning}  NOT FOUND ${chalk.greenBright(action)} KEY`
        );
    ...
    case 'set':
      if (!value)
        return console.log(
          `${logSymbols.warning}  Please set ${action} a value`
        );
    ...
    case 'delete':
      ...
      console.log(
        `${logSymbols.success} ${chalk.redBright(action)} deleted successfully `
      );
      break;
  }
}
